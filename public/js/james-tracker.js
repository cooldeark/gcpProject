(function() {
    //I hope will not to modified this shit until I leaving.2020/2/4
    if (typeof window.bbkkbbk !== 'object') {
        window.bbkkbbk = (function() {
            'use strict';
            var bbkkbbk,
                pentList = [],
                applyFirst = //我tracker載完之後，要先執行的東東順序，在initial的時候，在applyFirst裡的陣列參數為最優先
                ['addTracker',
                    'setTrackerUrl',
                    'setDebug',
                    'setEnabeCoookie',
                ...
                debug = false,
                asyncTrackers = new Array();//一堆一堆的tracker位置
            ...
            var D = function(a, b) { return 0 == a.indexOf(b) };//看a的開頭是不是b，如果是就回傳true
            var en = function(a) {
            ...
            var stripWwwFromDomain = function(domain) {//擷取domain
                var fp = domain.split('.')[0];
                if ((fp === 'www')||(fp ===  'cdn') ||(fp ==='pikachu')) { return domain.substring(3); } else { return domain; }
            };
            var En = function(a, b) {
                if (b == "undefined" || b == "") {
                    return a
            ...
            var ae = function(a) {//解析url把裡面東西拆出來
                function b(a) {
            ...
            var loglog = function(message, color) {//寫logㄏㄏ
                if (debug || color.toLowerCase() == "error") {
                    color = color || "black";
            ...
            var pageInfo = {//取得使用者頁面的資訊
                ...
                getInfo: function() {
                    return [
                        ...
                        En("up=", this.getPath()+this.getQuery()),
                        this.getUTM(),
                    ]
                }
            };

            var st = function(){return deviceInfo.getInfo().concat(pageInfo.getInfo())};//使用者裝置訊息+使用者頁面訊息全部存在st裡面

            var eventObjetc = {//assemble function 有用到這個
                general: {
                    name: 'en',
                    action: 'ea',
                    label: 'el',
             ...
             * Send Tracker Core
             */
            var ta = function(a) {//此function很重要，是回傳tracker蒐集的資料給GCP
                //M其實就是document
                var b = M.createElement("img");//create <img src='fuckyouPornHub'>
                b.width = 1;
                b.height = 1;
                b.src = a + "&z=" + getRandomNumber() + "&t=" + getISOTime();//發imgCode，就是把tracker.js蒐集來的userDeviceInfo & userPageInfo經由此ta function 發回GCP
                loglog('Send img tracker url : ' + b.src, 'success')
                return b//回傳回去的東西是 => <Img src='....'>
            };

            var wc = function(a, b, c) {
                var d = ta(a + "?" + b);
                var cb = (typeof c == 'function') ? c : df;
                d.onload = d.onerror = function() {//就是將我們的ta function 真正的呼叫去載圖
                    d.onload = null;
                    d.onerror = null;
                    cb()//callback funciton ，就是我們的c 其實是一個外來function
                }
            };

            var cutQuery = function(a, b, c) {//如果url超過500，就剪他老母
                var re = /[?&#]/;
                b = b.split(re);
            ...
            //將你的事件&資料組合起來
            var ba = function(a, b, c, d) {//a=> event name , b=>properties(有第一方第三方UID跟siteID), c=>maybe is callback function, d=>trackerService
                if (void 0 === d) d = "https://pikachu.holmesmind.com/pixel.png";
            ...
            var cftcookie = function(a, b, c) {//a => your cookie name , b => value, c=>domain
                var pr = '_cft';
                a = pr + a;
                if ("undefined" == typeof b) {//檢查cookie是否有種過
                    var e = ";?" + a + "=([^;]*);?",
                        f = new RegExp(e);
                    return f.test(M.cookie) ? RegExp.$1 : null
                }
                null === b && (b = "", c = c || {},
                    c.maxage = -1);// maxage=-1 就是刪掉此cookie
                var d = a + "=" + b;
                //下面的意思是 : if(c){ if(c.path){......}}一層一層判斷下去，不是 if(c &&c.path)醬子的判斷式
                c && (c.path && (d += "; path=" + c.path), c.maxage && (d += "; max-age=" + c.maxage), c.domain && (d += "; domain=" + c.domain), c.secure && (d += "; secure")),
                    M.cookie = d
            };

            var creatUid = function() {
                function s4() {//string four =>四個0~F之間的值
            ...
            function setMethods(q) {//執行你們外面的function Ex: cft('setSiteId','幹你娘') ， cft('setEnableCookie')...
                ...
                if (context[f]) {
                    context[f].apply(context, parameterArray);//f有可能是 function Tracker 的 setSiteId function ，這也會被其他funciton呼叫 Ex: sendPageView之類的
                } else {
                    loglog("Not Default Method : " + f, 'info');
                }
            }

            function applyMethods(q) {//這裡是讓最優先級的陣列參數先跑=>tracker載完之後，要先執行的東東順序，在initial的時候，在applyFirst裡的陣列參數為最優先
                var appliedMethods = bbkkbbk.getAsyncTrackers()[0].getAppliedMethods || {};//這裡只防最優先級陣列參數被重複設置 Ex: cft('setSiteID','fuck') cft('setSiteID','fuck') 如果有兩次，他只會認一次而已
                ...
                for (i = 0; i < applyFirst.length; i++) {
                    ...
                    for (j = 0; j < q.length; j++) {//cft.q ，這裡的q是 cft('addTracker') 儲存的咚咚，所以如果有兩個cft('addTracker')它會讓他剩一個
                        if (q[j] && q[j][0]) {
            ...
            var getScrollTop = function() {//Math.max是取你塞給他所有參數裡面的最大值 
                var getDocumentHeight = Math.max(M.documentElement.clientHeight,
                    M.documentElement.scrollHeight,
                    M.documentElement.offsetHeight,
                    M.body.scrollHeight,
            ...
            function Tracker(siteId) {
                ...
                function setSiteId(siteID, callback) {//這裡會被setMethod的context使用
                    ...
                    setUid();
                    setTPUid();
                    loglog('cft_uid='+cft_uid+'cf_p='+_cf_P,'debug');
                    pageview(callback);

                }


                function pageview(c) {//sendPageView，就是你再埋扣的地方會看到 analytics?en=pageview那個咚咚
                    if (sID == "") {
                    ...
                    spageview = checkInfo;
                    var xz = [];
                    xz.push('cftuid=' + cft_uid);//第一方UID
                    xz.push('cf_p=' + _cf_P);//第三方UID
                    xz.push('sid=' + sID);//siteID
                ...
                function setUid(costomerUid) {
                    cft_uid = (costomerUid ? costomerUid : cftcookie('_uid') ? cftcookie('_uid') : cft_uid ? cft_uid : creatUid());//creatUid 就是那個8-4-4-4-12
                }
                function setTPUid(TPUid) {//setThirdPartyUid , 2020/2/4 Alan : Un~~~~~~~ :D
                    _cf_P = (TPUid ? TPUid : cftcookie('_P') ? cftcookie('_P') : '' );
 
                }
               

                function useCookie() {//檢查是否能種哭ㄎ一，如果可以種的話，就回傳true(enableCookie)

                    if (navigator.cookieEnabled) {//navigator.cookieEnabled這意思是，能否種哭ㄎ一
                        // cftcookie('_test', 1, {
                        //     domain: stripWwwFromDomain(pageInfo.getHostName()),
                        //     maxage: 86400,
                        //     path: "/"
                        ...
                        if (cftcookie('_test')) {
                            ...
                            cftcookie('_test', null);//先測試是否可以種哭ㄎ一，可以就把原先value 1 清除
                ...
                function assemble(o, k, c) {//發送資料的用途 Ex: send pageView or sendEvent or sendPercentTage
                    //o => object , k=> event['genenal'] ,c=>callback
                    var checkInfo = "";
                    checkInfo = JSON.stringify(pageInfo.getUrlInfo()) + sID;//sID=siteID

                    if (spageview !== checkInfo) {//spageView=>site_pageView
                        loglog('This page hasn\'t send pageview', 'error');
                        return false;
                    };
                    var r = new Array,
                        d = {},
                        p = { items: !0 },//!0=true , !1=false , 2020/2/5 Alan:可能是抄來的吧ㄏㄏ我也忘記為啥要這樣寫了
                        // track_string = '',
                        xz = [],
                        i, j;
                    for (i in o) {
                    ...
                    ba(r, xz, c, trackerServer);// ba 94發送
                ...
                this.setEnableCookie = function(costomerUid, callback) {//種第一方的UID
                    if (useCookie()) {
                        setUid(costomerUid);
                        cftcookie('_uid', cft_uid, {
                            domain: stripWwwFromDomain(pageInfo.getHostName()),
                ...
                this.setTPCookie = function(){//種第三方的UID，就是廣告的UID
                    if (useCookie()) {
                ...
                this.setViewPercentage = function(viewPercentage = [30, 60, 90], callback) {
                    ...
                    W.addEventListener('scroll', viewScroll);
            ...
            function TrackerProxy() {//正式處理資料，他是cft真正的樣貌
                return function() {
                    var arg = [];
...
/*
這裡先不管誰去呼叫他，這裡的creatTracker
主要是把外面的cft.q資料拿進來用
*/
            function creatTracker(siteId) {
                var tracker = new Tracker(siteId);//將之實體化，因為裡面有很多function
                var q = [];
                if (typeof cft !== 'undefined' && typeof cft.q !== 'undefined') { q = cft.q };
                asyncTrackers.push(tracker);
                pentList = applyMethods(q);//pentList=pendingList的意思 2020/2/4 Alan : QQ，輪家拼錯惹ㄇQAQ
